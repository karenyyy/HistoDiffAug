...
class DDIMSampler(object):
    ...
    def sample(self,
        ...
        samples, intermediates = self.ddim_sampling(cond_fn, model_kwargs, conditioning, size,
                                                    callback=callback,
    ...
    @torch.no_grad()
    def ddim_sampling(self, cond_fn, model_kwargs, cond, shape,
    ...
    def _predict_eps_from_xstart(self, x_t, t, pred_xstart):
        return (
            self._extract_into_tensor(self.model.sqrt_recip_alphas_cumprod, t, x_t.shape) * x_t
            - pred_xstart
        ) / self._extract_into_tensor(self.model.sqrt_recipm1_alphas_cumprod, t, x_t.shape)


    @torch.no_grad()
    def p_sample_ddim(self, cond_fn, model_kwargs, x, c, t, total_steps, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
                      temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None,
                      unconditional_guidance_scale=1., unconditional_conditioning=None):
